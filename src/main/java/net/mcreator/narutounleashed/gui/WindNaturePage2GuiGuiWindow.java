...
import net.mcreator.narutounleashed.procedures.SmallWindBurstLearntTextProcedure;
...
@OnlyIn(Dist.CLIENT)
public class WindNaturePage2GuiGuiWindow extends ContainerScreen<WindNaturePage2GuiGui.GuiContainerMod> {
	private World world;
	private int x, y, z;
	private PlayerEntity entity;
	private final static HashMap guistate = WindNaturePage2GuiGui.guistate;

	public WindNaturePage2GuiGuiWindow(WindNaturePage2GuiGui.GuiContainerMod container, PlayerInventory inventory, ITextComponent text) {
	...
	private static final ResourceLocation texture = new ResourceLocation("naruto_unleashed:textures/wind_nature_page_2_gui.png");
	...
	protected void drawGuiContainerForegroundLayer(MatrixStack ms, int mouseX, int mouseY) {
		...
		this.font.drawString(ms, "Flying Technique", -29, 43, -14580090);
		...
		this.font.drawString(ms, "Small Wind Blast Jutsu", 87, 70, -16737155);
		...
		if (SmallWindBurstLearntTextProcedure.executeProcedure(Stream.of(new AbstractMap.SimpleEntry<>("entity", entity)).collect(HashMap::new,
				(_m, _e) -> _m.put(_e.getKey(), _e.getValue()), Map::putAll)))
	...
	public void init(Minecraft minecraft, int width, int height) {
		...
		this.addButton(new Button(this.guiLeft + -29, this.guiTop + 88, 30, 20, new StringTextComponent("Learn"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new WindNaturePage2GuiGui.ButtonPressedMessage(0, x, y, z));
				WindNaturePage2GuiGui.handleButtonAction(entity, 0, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 141, this.guiTop + 151, 30, 20, new StringTextComponent("Back"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new WindNaturePage2GuiGui.ButtonPressedMessage(1, x, y, z));
				WindNaturePage2GuiGui.handleButtonAction(entity, 1, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 87, this.guiTop + 115, 30, 20, new StringTextComponent("Learn"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new WindNaturePage2GuiGui.ButtonPressedMessage(2, x, y, z));
				WindNaturePage2GuiGui.handleButtonAction(entity, 2, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 177, this.guiTop + 151, 30, 20, new StringTextComponent("Next"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new WindNaturePage2GuiGui.ButtonPressedMessage(3, x, y, z));
				WindNaturePage2GuiGui.handleButtonAction(entity, 3, x, y, z);
			}
		}));
	}
}
