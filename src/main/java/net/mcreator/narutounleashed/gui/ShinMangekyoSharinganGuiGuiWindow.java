...
import net.mcreator.narutounleashed.procedures.ShinMsLearntTextProcedure;
...
@OnlyIn(Dist.CLIENT)
public class ShinMangekyoSharinganGuiGuiWindow extends ContainerScreen<ShinMangekyoSharinganGuiGui.GuiContainerMod> {
	private World world;
	private int x, y, z;
	private PlayerEntity entity;
	private final static HashMap guistate = ShinMangekyoSharinganGuiGui.guistate;

	public ShinMangekyoSharinganGuiGuiWindow(ShinMangekyoSharinganGuiGui.GuiContainerMod container, PlayerInventory inventory, ITextComponent text) {
	...
	private static final ResourceLocation texture = new ResourceLocation("naruto_unleashed:textures/shin_mangekyo_sharingan_gui.png");
	...
	protected void drawGuiContainerForegroundLayer(MatrixStack ms, int mouseX, int mouseY) {
		this.font.drawString(ms, "Shin", -29, -2, -3407872);
		...
		if (ShinMsLearntTextProcedure.executeProcedure(Stream.of(new AbstractMap.SimpleEntry<>("entity", entity)).collect(HashMap::new,
		...
		this.font.drawString(ms, "Telekinesis", 132, 34, -16777216);
		this.font.drawString(ms, "Susano", 159, 79, -1);
	...
	public void init(Minecraft minecraft, int width, int height) {
		...
		this.addButton(new Button(this.guiLeft + 168, this.guiTop + 151, 45, 20, new StringTextComponent("Back"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new ShinMangekyoSharinganGuiGui.ButtonPressedMessage(0, x, y, z));
				ShinMangekyoSharinganGuiGui.handleButtonAction(entity, 0, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 51, this.guiTop + 70, 30, 20, new StringTextComponent("Learn"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new ShinMangekyoSharinganGuiGui.ButtonPressedMessage(1, x, y, z));
				ShinMangekyoSharinganGuiGui.handleButtonAction(entity, 1, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 132, this.guiTop + 52, 35, 20, new StringTextComponent("Go"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new ShinMangekyoSharinganGuiGui.ButtonPressedMessage(2, x, y, z));
				ShinMangekyoSharinganGuiGui.handleButtonAction(entity, 2, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 159, this.guiTop + 97, 30, 20, new StringTextComponent("Go"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new ShinMangekyoSharinganGuiGui.ButtonPressedMessage(3, x, y, z));
				ShinMangekyoSharinganGuiGui.handleButtonAction(entity, 3, x, y, z);
			}
		}));
	}
}
