...
import net.mcreator.narutounleashed.procedures.MadaraMSLearntTextProcedure;
...
@OnlyIn(Dist.CLIENT)
public class MMSGuiGuiWindow extends ContainerScreen<MMSGuiGui.GuiContainerMod> {
	private World world;
	private int x, y, z;
	private PlayerEntity entity;
	private final static HashMap guistate = MMSGuiGui.guistate;

	public MMSGuiGuiWindow(MMSGuiGui.GuiContainerMod container, PlayerInventory inventory, ITextComponent text) {
	...
	private static final ResourceLocation texture = new ResourceLocation("naruto_unleashed:textures/mms_gui.png");
	...
	protected void drawGuiContainerForegroundLayer(MatrixStack ms, int mouseX, int mouseY) {
		this.font.drawString(ms, "Madara", -29, -2, -3407872);
		...
		if (MadaraMSLearntTextProcedure.executeProcedure(Stream.of(new AbstractMap.SimpleEntry<>("entity", entity)).collect(HashMap::new,
		...
		this.font.drawString(ms, "Bijuu Control", 132, 34, -16777216);
		this.font.drawString(ms, "Susano", 159, 79, -16777063);
		...
		this.font.drawString(ms, "coming soon...", 132, 46, -12829636);
	}
	...
	public void init(Minecraft minecraft, int width, int height) {
		...
		this.addButton(new Button(this.guiLeft + 168, this.guiTop + 151, 45, 20, new StringTextComponent("Back"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new MMSGuiGui.ButtonPressedMessage(0, x, y, z));
				MMSGuiGui.handleButtonAction(entity, 0, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 51, this.guiTop + 70, 30, 20, new StringTextComponent("Learn"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new MMSGuiGui.ButtonPressedMessage(1, x, y, z));
				MMSGuiGui.handleButtonAction(entity, 1, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 159, this.guiTop + 97, 30, 20, new StringTextComponent("Go"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new MMSGuiGui.ButtonPressedMessage(2, x, y, z));
				MMSGuiGui.handleButtonAction(entity, 2, x, y, z);
			}
		}));
	}
}
