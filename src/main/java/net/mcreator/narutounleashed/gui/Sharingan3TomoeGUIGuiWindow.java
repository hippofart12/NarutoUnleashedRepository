...
import net.mcreator.narutounleashed.procedures.ThTSlearnttextProcedure;
...
@OnlyIn(Dist.CLIENT)
public class Sharingan3TomoeGUIGuiWindow extends ContainerScreen<Sharingan3TomoeGUIGui.GuiContainerMod> {
	private World world;
	private int x, y, z;
	private PlayerEntity entity;
	private final static HashMap guistate = Sharingan3TomoeGUIGui.guistate;

	public Sharingan3TomoeGUIGuiWindow(Sharingan3TomoeGUIGui.GuiContainerMod container, PlayerInventory inventory, ITextComponent text) {
	...
	private static final ResourceLocation texture = new ResourceLocation("naruto_unleashed:textures/sharingan_3_tomoe_gui.png");
	...
	protected void drawGuiContainerBackgroundLayer(MatrixStack ms, float partialTicks, int gx, int gy) {
		...
		Minecraft.getInstance().getTextureManager().bindTexture(new ResourceLocation("naruto_unleashed:textures/sharinganeye3.png"));
	...
	protected void drawGuiContainerForegroundLayer(MatrixStack ms, int mouseX, int mouseY) {
		...
		this.font.drawString(ms, "Tomoe 3", -29, 7, -16777216);
		...
		this.font.drawString(ms, "Requires: 20,000 Jutsu Points", -29, 43, -12829636);
		if (ThTSlearnttextProcedure.executeProcedure(Stream.of(new AbstractMap.SimpleEntry<>("entity", entity)).collect(HashMap::new,
				(_m, _e) -> _m.put(_e.getKey(), _e.getValue()), Map::putAll)))
			this.font.drawString(ms, "Learnt", 87, 70, -12829636);
		this.font.drawString(ms, "Jutsu Points:", 78, -2, -12829636);
		this.font.drawString(ms, "" + (int) ((entity.getCapability(NarutoUnleashedModVariables.PLAYER_VARIABLES_CAPABILITY, null)
				.orElse(new NarutoUnleashedModVariables.PlayerVariables())).JutsuPoints) + "", 141, -2, -12829636);
	}
	...
	public void init(Minecraft minecraft, int width, int height) {
		...
		this.addButton(new Button(this.guiLeft + 168, this.guiTop + 151, 45, 20, new StringTextComponent("Back"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new Sharingan3TomoeGUIGui.ButtonPressedMessage(0, x, y, z));
				Sharingan3TomoeGUIGui.handleButtonAction(entity, 0, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 51, this.guiTop + 70, 30, 20, new StringTextComponent("Learn"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new Sharingan3TomoeGUIGui.ButtonPressedMessage(1, x, y, z));
				Sharingan3TomoeGUIGui.handleButtonAction(entity, 1, x, y, z);
			}
		}));
	}
}
