...
@OnlyIn(Dist.CLIENT)
public class WindNatureGui12GuiWindow extends ContainerScreen<WindNatureGui12Gui.GuiContainerMod> {
	private World world;
	private int x, y, z;
	private PlayerEntity entity;
	private final static HashMap guistate = WindNatureGui12Gui.guistate;

	public WindNatureGui12GuiWindow(WindNatureGui12Gui.GuiContainerMod container, PlayerInventory inventory, ITextComponent text) {
	...
	private static final ResourceLocation texture = new ResourceLocation("naruto_unleashed:textures/wind_nature_gui_12.png");
	...
	protected void drawGuiContainerForegroundLayer(MatrixStack ms, int mouseX, int mouseY) {
		...
		this.font.drawString(ms, "Wind Tunnel Jutsu", -29, 43, -14580090);
		...
		this.font.drawString(ms, "Wind Flower Missile Jutsu", 87, 70, -16737155);
	...
	public void init(Minecraft minecraft, int width, int height) {
		...
		this.addButton(new Button(this.guiLeft + -29, this.guiTop + 88, 30, 20, new StringTextComponent("Learn"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new WindNatureGui12Gui.ButtonPressedMessage(0, x, y, z));
				WindNatureGui12Gui.handleButtonAction(entity, 0, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 178, this.guiTop + 151, 30, 20, new StringTextComponent("Back"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new WindNatureGui12Gui.ButtonPressedMessage(1, x, y, z));
				WindNatureGui12Gui.handleButtonAction(entity, 1, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 87, this.guiTop + 115, 30, 20, new StringTextComponent("Learn"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new WindNatureGui12Gui.ButtonPressedMessage(2, x, y, z));
				WindNatureGui12Gui.handleButtonAction(entity, 2, x, y, z);
			}
		}));
	}
}
