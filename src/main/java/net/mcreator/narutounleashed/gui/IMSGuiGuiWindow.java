...
import net.mcreator.narutounleashed.procedures.IzunaMSLearntTextProcedure;
...
@OnlyIn(Dist.CLIENT)
public class IMSGuiGuiWindow extends ContainerScreen<IMSGuiGui.GuiContainerMod> {
	private World world;
	private int x, y, z;
	private PlayerEntity entity;
	private final static HashMap guistate = IMSGuiGui.guistate;

	public IMSGuiGuiWindow(IMSGuiGui.GuiContainerMod container, PlayerInventory inventory, ITextComponent text) {
	...
	private static final ResourceLocation texture = new ResourceLocation("naruto_unleashed:textures/ims_gui.png");
	...
	protected void drawGuiContainerForegroundLayer(MatrixStack ms, int mouseX, int mouseY) {
		this.font.drawString(ms, "Izuna", -29, -2, -3407872);
		...
		if (IzunaMSLearntTextProcedure.executeProcedure(Stream.of(new AbstractMap.SimpleEntry<>("entity", entity)).collect(HashMap::new,
		...
		this.font.drawString(ms, "Kenjutsu Skill", 132, 34, -16777216);
		this.font.drawString(ms, "Susano", 159, 79, -16750900);
	...
	public void init(Minecraft minecraft, int width, int height) {
		...
		this.addButton(new Button(this.guiLeft + 168, this.guiTop + 151, 45, 20, new StringTextComponent("Back"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new IMSGuiGui.ButtonPressedMessage(0, x, y, z));
				IMSGuiGui.handleButtonAction(entity, 0, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 51, this.guiTop + 70, 30, 20, new StringTextComponent("Learn"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new IMSGuiGui.ButtonPressedMessage(1, x, y, z));
				IMSGuiGui.handleButtonAction(entity, 1, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 132, this.guiTop + 52, 30, 20, new StringTextComponent("Go"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new IMSGuiGui.ButtonPressedMessage(2, x, y, z));
				IMSGuiGui.handleButtonAction(entity, 2, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 159, this.guiTop + 97, 30, 20, new StringTextComponent("Go"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new IMSGuiGui.ButtonPressedMessage(3, x, y, z));
				IMSGuiGui.handleButtonAction(entity, 3, x, y, z);
			}
		}));
	}
}
