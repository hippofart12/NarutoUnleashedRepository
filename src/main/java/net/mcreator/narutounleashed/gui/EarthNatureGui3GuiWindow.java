...
import net.mcreator.narutounleashed.procedures.AttackingPlantsLearntTextProcedure;
...
@OnlyIn(Dist.CLIENT)
public class EarthNatureGui3GuiWindow extends ContainerScreen<EarthNatureGui3Gui.GuiContainerMod> {
	private World world;
	private int x, y, z;
	private PlayerEntity entity;
	private final static HashMap guistate = EarthNatureGui3Gui.guistate;

	public EarthNatureGui3GuiWindow(EarthNatureGui3Gui.GuiContainerMod container, PlayerInventory inventory, ITextComponent text) {
	...
	private static final ResourceLocation texture = new ResourceLocation("naruto_unleashed:textures/earth_nature_gui_3.png");
	...
	protected void drawGuiContainerForegroundLayer(MatrixStack ms, int mouseX, int mouseY) {
		...
		this.font.drawString(ms, "Attacking Plants Jutsu", -29, 43, -7589376);
		this.font.drawString(ms, "1100 Jutsu Points", -11, 52, -10274033);
		this.font.drawString(ms, "Jutsu Combo: 157", -11, 61, -7779311);
		this.font.drawString(ms, "157", -29, 70, -12829636);
		this.font.drawString(ms, "400 Chakra", -2, 70, -13429248);
		if (AttackingPlantsLearntTextProcedure.executeProcedure(Stream.of(new AbstractMap.SimpleEntry<>("entity", entity)).collect(HashMap::new,
		...
		this.font.drawString(ms, "Stone Petrify Jutsu", 87, 70, -10079488);
	...
	public void init(Minecraft minecraft, int width, int height) {
		...
		this.addButton(new Button(this.guiLeft + -29, this.guiTop + 88, 30, 20, new StringTextComponent("Learn"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new EarthNatureGui3Gui.ButtonPressedMessage(0, x, y, z));
				EarthNatureGui3Gui.handleButtonAction(entity, 0, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 177, this.guiTop + 151, 30, 20, new StringTextComponent("Back"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new EarthNatureGui3Gui.ButtonPressedMessage(1, x, y, z));
				EarthNatureGui3Gui.handleButtonAction(entity, 1, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 87, this.guiTop + 115, 30, 20, new StringTextComponent("Learn"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new EarthNatureGui3Gui.ButtonPressedMessage(2, x, y, z));
				EarthNatureGui3Gui.handleButtonAction(entity, 2, x, y, z);
			}
		}));
	}
}
