...
import net.mcreator.narutounleashed.procedures.TrueSightMSLearntTextProcedure;
...
@OnlyIn(Dist.CLIENT)
public class TrueSightMSGuiGuiWindow extends ContainerScreen<TrueSightMSGuiGui.GuiContainerMod> {
	private World world;
	private int x, y, z;
	private PlayerEntity entity;
	private final static HashMap guistate = TrueSightMSGuiGui.guistate;

	public TrueSightMSGuiGuiWindow(TrueSightMSGuiGui.GuiContainerMod container, PlayerInventory inventory, ITextComponent text) {
	...
	private static final ResourceLocation texture = new ResourceLocation("naruto_unleashed:textures/true_sight_ms_gui.png");
	...
	protected void drawGuiContainerBackgroundLayer(MatrixStack ms, float partialTicks, int gx, int gy) {
		...
		Minecraft.getInstance().getTextureManager().bindTexture(new ResourceLocation("naruto_unleashed:textures/true_sight_mangekyo.png"));
	...
	protected void drawGuiContainerForegroundLayer(MatrixStack ms, int mouseX, int mouseY) {
		this.font.drawString(ms, "True Sight", -29, -2, -3407872);
		...
		if (TrueSightMSLearntTextProcedure.executeProcedure(Stream.of(new AbstractMap.SimpleEntry<>("entity", entity)).collect(HashMap::new,
		...
		this.font.drawString(ms, "Lightning", 132, 34, -13312);
	...
	public void init(Minecraft minecraft, int width, int height) {
		...
		this.addButton(new Button(this.guiLeft + 168, this.guiTop + 151, 45, 20, new StringTextComponent("Back"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new TrueSightMSGuiGui.ButtonPressedMessage(0, x, y, z));
				TrueSightMSGuiGui.handleButtonAction(entity, 0, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 51, this.guiTop + 70, 30, 20, new StringTextComponent("Learn"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new TrueSightMSGuiGui.ButtonPressedMessage(1, x, y, z));
				TrueSightMSGuiGui.handleButtonAction(entity, 1, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 132, this.guiTop + 52, 35, 20, new StringTextComponent("Go"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new TrueSightMSGuiGui.ButtonPressedMessage(2, x, y, z));
				TrueSightMSGuiGui.handleButtonAction(entity, 2, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 159, this.guiTop + 97, 30, 20, new StringTextComponent("Go"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new TrueSightMSGuiGui.ButtonPressedMessage(3, x, y, z));
				TrueSightMSGuiGui.handleButtonAction(entity, 3, x, y, z);
			}
		}));
	}
}
