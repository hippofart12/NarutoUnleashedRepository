...
@OnlyIn(Dist.CLIENT)
public class EarthNatureGui13GuiWindow extends ContainerScreen<EarthNatureGui13Gui.GuiContainerMod> {
	private World world;
	private int x, y, z;
	private PlayerEntity entity;
	private final static HashMap guistate = EarthNatureGui13Gui.guistate;

	public EarthNatureGui13GuiWindow(EarthNatureGui13Gui.GuiContainerMod container, PlayerInventory inventory, ITextComponent text) {
	...
	private static final ResourceLocation texture = new ResourceLocation("naruto_unleashed:textures/earth_nature_gui_13.png");
	...
	protected void drawGuiContainerForegroundLayer(MatrixStack ms, int mouseX, int mouseY) {
		...
		this.font.drawString(ms, "Underground Projection Fish Technique", -29, 43, -7589376);
		...
		this.font.drawString(ms, "Inner Decapitation Technique Jutsu", 87, 70, -10079488);
	...
	public void init(Minecraft minecraft, int width, int height) {
		...
		this.addButton(new Button(this.guiLeft + -29, this.guiTop + 88, 30, 20, new StringTextComponent("Learn"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new EarthNatureGui13Gui.ButtonPressedMessage(0, x, y, z));
				EarthNatureGui13Gui.handleButtonAction(entity, 0, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 177, this.guiTop + 151, 30, 20, new StringTextComponent("Back"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new EarthNatureGui13Gui.ButtonPressedMessage(1, x, y, z));
				EarthNatureGui13Gui.handleButtonAction(entity, 1, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 87, this.guiTop + 115, 30, 20, new StringTextComponent("Learn"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new EarthNatureGui13Gui.ButtonPressedMessage(2, x, y, z));
				EarthNatureGui13Gui.handleButtonAction(entity, 2, x, y, z);
			}
		}));
	}
}
