...
@OnlyIn(Dist.CLIENT)
public class EarthNatureGui2GuiWindow extends ContainerScreen<EarthNatureGui2Gui.GuiContainerMod> {
	private World world;
	private int x, y, z;
	private PlayerEntity entity;
	private final static HashMap guistate = EarthNatureGui2Gui.guistate;

	public EarthNatureGui2GuiWindow(EarthNatureGui2Gui.GuiContainerMod container, PlayerInventory inventory, ITextComponent text) {
	...
	private static final ResourceLocation texture = new ResourceLocation("naruto_unleashed:textures/earth_nature_gui_2.png");
	...
	protected void drawGuiContainerForegroundLayer(MatrixStack ms, int mouseX, int mouseY) {
		...
		this.font.drawString(ms, "Rock Rain Boulder Jutsu", 87, 70, -10079488);
	...
	public void init(Minecraft minecraft, int width, int height) {
		...
		this.addButton(new Button(this.guiLeft + -29, this.guiTop + 88, 30, 20, new StringTextComponent("Learn"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new EarthNatureGui2Gui.ButtonPressedMessage(0, x, y, z));
				EarthNatureGui2Gui.handleButtonAction(entity, 0, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 141, this.guiTop + 151, 30, 20, new StringTextComponent("Back"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new EarthNatureGui2Gui.ButtonPressedMessage(1, x, y, z));
				EarthNatureGui2Gui.handleButtonAction(entity, 1, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 87, this.guiTop + 115, 30, 20, new StringTextComponent("Learn"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new EarthNatureGui2Gui.ButtonPressedMessage(2, x, y, z));
				EarthNatureGui2Gui.handleButtonAction(entity, 2, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 177, this.guiTop + 151, 30, 20, new StringTextComponent("Next"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new EarthNatureGui2Gui.ButtonPressedMessage(3, x, y, z));
				EarthNatureGui2Gui.handleButtonAction(entity, 3, x, y, z);
			}
		}));
	}
}
