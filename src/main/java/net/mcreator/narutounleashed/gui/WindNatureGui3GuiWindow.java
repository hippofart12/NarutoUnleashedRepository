...
import net.mcreator.narutounleashed.procedures.WirlwindPrisonLearntTextProcedure;
...
@OnlyIn(Dist.CLIENT)
public class WindNatureGui3GuiWindow extends ContainerScreen<WindNatureGui3Gui.GuiContainerMod> {
	private World world;
	private int x, y, z;
	private PlayerEntity entity;
	private final static HashMap guistate = WindNatureGui3Gui.guistate;

	public WindNatureGui3GuiWindow(WindNatureGui3Gui.GuiContainerMod container, PlayerInventory inventory, ITextComponent text) {
	...
	private static final ResourceLocation texture = new ResourceLocation("naruto_unleashed:textures/wind_nature_gui_3.png");
	...
	protected void drawGuiContainerForegroundLayer(MatrixStack ms, int mouseX, int mouseY) {
		...
		this.font.drawString(ms, "Whirlwind Prison Jutsu", -29, 43, -14580090);
		this.font.drawString(ms, "1100 Jutsu Points", -11, 52, -15368321);
		this.font.drawString(ms, "Jutsu Combo: 436", -11, 61, -15960180);
		this.font.drawString(ms, "436", -29, 70, -12829636);
		this.font.drawString(ms, "3500 Chakra", -2, 70, -16152408);
		this.font.drawString(ms, "Requires: 90 Ninjutsu", -29, 79, -12829636);
		if (WirlwindPrisonLearntTextProcedure.executeProcedure(Stream.of(new AbstractMap.SimpleEntry<>("entity", entity)).collect(HashMap::new,
				(_m, _e) -> _m.put(_e.getKey(), _e.getValue()), Map::putAll)))
			this.font.drawString(ms, "Learnt", 6, 88, -12829636);
		this.font.drawString(ms, "Great Wind Blast Jutsu", 87, 70, -16737155);
	...
	public void init(Minecraft minecraft, int width, int height) {
		...
		this.addButton(new Button(this.guiLeft + -29, this.guiTop + 88, 30, 20, new StringTextComponent("Learn"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new WindNatureGui3Gui.ButtonPressedMessage(0, x, y, z));
				WindNatureGui3Gui.handleButtonAction(entity, 0, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 178, this.guiTop + 151, 30, 20, new StringTextComponent("Back"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new WindNatureGui3Gui.ButtonPressedMessage(1, x, y, z));
				WindNatureGui3Gui.handleButtonAction(entity, 1, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 87, this.guiTop + 115, 30, 20, new StringTextComponent("Learn"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new WindNatureGui3Gui.ButtonPressedMessage(2, x, y, z));
				WindNatureGui3Gui.handleButtonAction(entity, 2, x, y, z);
			}
		}));
	}
}
