...
@OnlyIn(Dist.CLIENT)
public class WindNatureGui9GuiWindow extends ContainerScreen<WindNatureGui9Gui.GuiContainerMod> {
	private World world;
	private int x, y, z;
	private PlayerEntity entity;
	private final static HashMap guistate = WindNatureGui9Gui.guistate;

	public WindNatureGui9GuiWindow(WindNatureGui9Gui.GuiContainerMod container, PlayerInventory inventory, ITextComponent text) {
	...
	private static final ResourceLocation texture = new ResourceLocation("naruto_unleashed:textures/wind_nature_gui_9.png");
	...
	public void init(Minecraft minecraft, int width, int height) {
		...
		this.addButton(new Button(this.guiLeft + -29, this.guiTop + 88, 30, 20, new StringTextComponent("Learn"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new WindNatureGui9Gui.ButtonPressedMessage(0, x, y, z));
				WindNatureGui9Gui.handleButtonAction(entity, 0, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 178, this.guiTop + 151, 30, 20, new StringTextComponent("Back"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new WindNatureGui9Gui.ButtonPressedMessage(1, x, y, z));
				WindNatureGui9Gui.handleButtonAction(entity, 1, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 87, this.guiTop + 115, 30, 20, new StringTextComponent("Learn"), e -> {
			if (true) {
				NarutoUnleashedMod.PACKET_HANDLER.sendToServer(new WindNatureGui9Gui.ButtonPressedMessage(2, x, y, z));
				WindNatureGui9Gui.handleButtonAction(entity, 2, x, y, z);
			}
		}));
	}
}
