...
public class CrystalParticlesParticle {
	...
	public static void registerParticleType(RegistryEvent.Register<ParticleType<?>> event) {
		event.getRegistry().register(particle.setRegistryName("crystal_particles"));
	}
	...
	private static class CustomParticle extends SpriteTexturedParticle {
		...
		protected CustomParticle(ClientWorld world, double x, double y, double z, double vx, double vy, double vz, IAnimatedSprite spriteSet) {
			...
			this.maxAge = (int) Math.max(1, 4 + (this.rand.nextInt(4) - 2));
			this.particleGravity = (float) 0.2;
			this.canCollide = false;
			this.motionX = vx * 2;
			this.motionY = vy * 2;
			this.motionZ = vz * 2;
			this.angularVelocity = (float) 6.5;
			this.angularAcceleration = (float) 0.2;
