...
package net.mcreator.narutounleashed.particle;
...
public class FirstThroughSixthGateParticleParticle {
	...
	public static void registerParticleType(RegistryEvent.Register<ParticleType<?>> event) {
		event.getRegistry().register(particle.setRegistryName("first_through_sixth_gate_particle"));
	}
	...
	private static class CustomParticle extends SpriteTexturedParticle {
		...
		protected CustomParticle(ClientWorld world, double x, double y, double z, double vx, double vy, double vz, IAnimatedSprite spriteSet) {
			...
			this.setSize((float) 0.4, (float) 0.5);
			this.particleScale *= (float) 1.7000000000000002;
			this.maxAge = (int) Math.max(1, 2 + (this.rand.nextInt(8) - 4));
			this.particleGravity = (float) -2.2;
